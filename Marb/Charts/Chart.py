...
class Axis:
	def __init__( self, nbTicks = 10, min = 0, max = 0 ):
		self.min = min
		self.max = max
	...
	def setAlphaBeta(self, length):
		...
		'''
		self.length = length
		self.alpha = -float(self.length) / float( self.maxBound - self.minBound )
		self.beta = (self.maxBound * self.length ) / ( self.maxBound - self.minBound ) + self.axisPos.y()
