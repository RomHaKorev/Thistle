...
class LinearChart(Chart):
	...
	def itemRect(self, index ):
		...
		if t == Type.Bar:
			...
			if self._yAxis.dataStartonYAxis == False:
	...
	def paintChart(self, painter):
		...
		labels = [ str(self.model().headerData( i, Qt.Vertical )) for i in range( self.model().rowCount() ) ]
		self._yAxis.paint( painter, labels )
	...
	def _paintTextAxis(self, painter):
		...
		while (i < self.model().rowCount() ):
			...
			if self._yAxis.verticalLabel == True:
				painter.rotate( -90 )
				if self._yAxis.dataStartonYAxis == False:
	...
	def _updateRects(self):
		...
		self._yAxis.verticalLabel = (length > self._yAxis.xStep)
		self._yAxis.axisPos = self._valuesRect.topLeft()
		self._yAxis.setAlphaBeta( self._valuesRect.height() - 20 )
