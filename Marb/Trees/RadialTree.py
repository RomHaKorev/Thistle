...
class RadialTree(Tree):
	...
	def displayCicrcle(self, display = True ):
		'''Defines if circle should be painted to specify the item depth.
		*display: boolean
		'''
	...
	def updatePerimeter(self):
		'''Update the perimeter needed to paint the tree.
		'''
	...
	def paintCircles(self, painter, offset):
		'''Paints circles that represents the item depth.
		'''
		painter.save()
