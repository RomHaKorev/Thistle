...
public:
	...
	static QColor predefinedLightColor( int key )
	{
		...
		{
			QString k( metaEnum.key( i ) );
			if ( k.startsWith("Light") == true ) colors << metaEnum.value( i );
		}
	...
	static QColor predefinedRegularColor( int key )
	{
		...
		{
			QString k( metaEnum.key( i ) );
			if ( (k.startsWith("Light") || k.startsWith("Dark") ) == false ) colors << metaEnum.value( i );
		}
	...
	static QColor predefinedDarkColor( int key )
	{
		...
		{
			QString k( metaEnum.key( i ) );
			if ( k.startsWith("Dark") == true ) colors << metaEnum.value( i );
		}
		if ( key < 0 || key >= colors.count() ) return QColor( 20, 20, 20 );
