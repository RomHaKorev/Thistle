...
GraphAlgorithm::GraphAlgorithm( GraphView* parent )
    : AbstractGraphAlgorithm( new GraphAlgorithmPrivate() , parent )
{
    Q_D( GraphAlgorithm );
...
void GraphAlgorithm::processTimer()
{
    ...
    if ( p == 0 )
    {
        qWarning() << "GraphAlgorithm should be used with GraphView class.";
        d->timer->stop();
        return;
    }
    ...
    d->realSize = poly.boundingRect().size() + ( p->itemRect( p->model()->index( 0, 0 ) ).size() );

    p->setNodeBoundingRect( poly.boundingRect().toRect() );
    emit updated();
